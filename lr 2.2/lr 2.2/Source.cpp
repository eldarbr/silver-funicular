...
void product(int** arr, int row, int col) {
	...
	for (int i = 0; i < row; i++) {
		...
		if (sign == 0) {
			...
			cout << "Произведение элементов " << i + 1 << " строки: не посчитано - содержится 0" << endl;
		}
		prod = 1;
		sign = 0;
	}
	if (max_prod == 0) {
		cout << "Максимальное произведение не найдено - все строки содержат 0" << endl << endl;
	}
	else {
		cout << "Максимальное произведение элементов строки: " << max_prod << endl << endl;
	}
}
